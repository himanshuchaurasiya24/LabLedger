...
final String staffsBaseUrl = '$globalBaseUrl${AppUrls.staffBase}';

typedef UserLockStatusInput = ({int userId, bool isLocked});

final usersDetailsProvider = FutureProvider.autoDispose<List<User>>((
  ref,
) async {
...
final singleUserDetailsProvider = FutureProvider.family.autoDispose<User, int>((
  ref,
  id,
) async {
...
final createUserProvider = FutureProvider.family
    .autoDispose<User, Map<String, dynamic>>((ref, userData) async {
...
final toggleUserLockStatusProvider = FutureProvider.family
    .autoDispose<User, UserLockStatusInput>((ref, params) async {
      final response = await AuthHttpClient.put(
        ref,
