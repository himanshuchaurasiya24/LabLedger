...
final String billGrowthStatsEndpoint = "${globalBaseUrl}diagnosis/bills/growth-stats/";
...
final latestBillsProvider = FutureProvider.autoDispose<List<Bill>>((ref) async {
  final uri = Uri.parse(billsEndpoint).replace(queryParameters: {
    'page': '1',
    'page_size': '10',
  });
...
final singleBillProvider =
    FutureProvider.autoDispose.family<Bill, int>((ref, id) async {
  final response = await AuthHttpClient.get(ref, "$billsEndpoint$id/?list_format=true");
...
final createBillProvider =
    FutureProvider.autoDispose.family<Bill, Bill>((ref, newBill) async {
...
final updateBillProvider =
    FutureProvider.autoDispose.family<Bill, Bill>((ref, updatedBill) async {
...
/*
  NOTE: The unpaginated `billProvider` was removed. Fetching an entire, unbounded list 
  of items is not a scalable practice and can lead to significant performance issues
  as the number of bills grows. The `paginatedBillsProvider` is the correct,
  performant replacement for this functionality.
*/